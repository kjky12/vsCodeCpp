...
/*     for (size_t i = 0; i < participant.size(); i++)
    ...
    vector<int> answer;
    int nIdx = heights.size();
    answer.resize(nIdx);

    for (vector<int>::reverse_iterator iterTmp = heights.rbegin(); iterTmp != heights.rend(); iterTmp++)
    {   
       int nIdx2 = nIdx;
        for (vector<int>::reverse_iterator iterTmp2 = iterTmp; iterTmp2 != heights.rend(); )
        {      
            iterTmp2++;
            nIdx2--;

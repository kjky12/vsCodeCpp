...
int GetSumValue(int nFir, int nSeco)
{
    return nFir + (nSeco * 2);
}
...
{   
   ...
   int K;
    ...
    //Case 1
    const int nTmp1 = 6;
    int naTmp1[nTmp1] = {1, 2, 3, 9, 10, 12};
    scoville.assign(naTmp1, naTmp1+nTmp1);
    ...
    while (true)
    {
        ...
        for (vector<int>::const_iterator iterTmp = scoville.begin(); iterTmp != scoville.end(); iterTmp++)
        {   
            int n = *iterTmp;
            if(K > *iterTmp)
            {
        ...
        else //! 스코빌 지수가 기준을 안넘는게 있다!
        {
            if(scoville.size() > 2)
            {
                scoville[0] = GetSumValue(scoville[0], scoville[1]);
                scoville.erase(scoville.begin() + 1);
            }
            else //! 두개가 안넘으면 합칠게 없다!
            {
                answer = -1;
                break;
            }
